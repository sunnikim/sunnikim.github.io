...
function preload() {
    ...
    myShader = loadShader(
        "07_vertex_shader/preset1/shader.vert",
        "07_vertex_shader/preset1/shader.frag"
    );
...
function setup() {
    ...
    for (let i = 0; i < 5; i++) {
        balls.push(new Ball(random(width),random(height),150))
    }
...
function draw() {
    ...
    let thresholdVolume = map(micVolume,0,0.5,0,5)
    ...
    myShader.setUniform("u_threshold", thresholdVolume);
