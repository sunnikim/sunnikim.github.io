    ...
    class Pixelator {
        ...
        drawSprite(x, y, w, h, c) {
            ...
            if (this.settings.type) {
                ...
                this.outGraphics.drawingContext.fillStyle =
                    "#" + hexC + hexC + hexC;
                this.outGraphics.drawingContext.fillRect(
                    (x * this.bW) | 0,
                    (y * this.bH) | 0,
                    (w * this.bW) | 0,
                    (h * this.bH) | 0
                );
                this.outGraphics.drawingContext.strokeRect(
                    (x * this.bW) | 0,
                    (y * this.bH) | 0,
                    (w * this.bW) | 0,
                    (h * this.bH) | 0
        ...
        newSprite(i) {
            ...
            if (this.settings.type == "image") {
                ...
                img =
                    random(1) > 0.5
                        ? createGraphics(128, 1)
                        : createGraphics(1, 128);
                img.pixelDensity(1);
                img.background(100);
                img.noStroke();
                img.loadPixels();
                ...
                switch (floor(random(5))) {
                    ...
                    case 1:
                        img.rect(0, 0, w * 0.25, h);
                        img.rect(0 + w * 0.5, 0, w * 0.25, h);
                        break;

                    case 2:
                        img.rect(0, 0, w * 0.333, h);
                        img.rect(0 + w * 0.666, 0, w * 0.333, h);
                        break;

                    case 3:
                        for (let i = 0; i < 3; i++) {
                            let d = 1 / 5;
                            img.rect(0 + w * d * i * 2, 0, w * d, h);
                        }
                        break;
                    ...
                    case 6:
                        rect(x, y, w, h * 0.25);
                        rect(x, y + h * 0.5, w, h * 0.25);
                        break;

                    case 7:
                        rect(x, y, w, h * 0.333);
                        rect(x, y + h * 0.666, w, h * 0.333);
                        break;

                    case 8:
                        for (let i = 0; i < 3; i++) {
                            let d = 1 / 5;
                            rect(x, y + h * d * i * 2, w, h * d);
                        }
                        break;
            ...
            if (img.canvas.parentElement)
                img.canvas.parentElement.removeChild(img.canvas);
            return img;
        }
        ...
        constructor(p, graphics, settings = {}) {
            ...
            this.bW = W / count;
            this.bH = Math.round(
                (this.bW * this.inGraphicsRaw.height) / this.inGraphicsRaw.width
            );
            H = this.bH * count;
            this.width = W;
            this.height = H;
        ...
        parent(p) {
            document
                .querySelector("#" + p)
                .appendChild(this.outGraphics.canvas);
        }
        ...
        set(settings = {}) {
            ...
            if (settings.tolerance != null)
                this.settings.tolerance = settings.tolerance;
            if (settings.type) {
                this.settings.type = settings.type;
                this.settings.palette =
                    settings.palette || this.settings.palette;
        ...
        update() {
            ...
            this.mouseX =
                (mouseX - this.outGraphics.canvas.offsetLeft) /
                this.outGraphics.canvas.offsetWidth;
            this.mouseY =
                (mouseY +
                    document.body.scrollTop -
                    this.outGraphics.canvas.offsetTop) /
                this.outGraphics.canvas.offsetHeight;
        }
